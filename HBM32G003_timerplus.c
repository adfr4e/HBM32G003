...
* 功能说明:	TIMPLUS公共参数初始化
* 输    入: TIMPLUS_TypeDef * TIMPLUSx                              指定要被设置的TIMPLUS，有效值包括TIMPLUS0
...
void TIMPLUS_SetPeriod(TIMPLUS_TypeDef * TIMPLUSx,TIMPLUS_TypeTypeDef type,uint16_t Period)
{
	...
	if((type & TIMPLUS_LOW) == TIMPLUS_LOW)
	{
		TIMPLUSx->LOW_PERIOD = Period;     //定时器周期配置值   实际周期值为配置值加1
	}
	
	if((type & TIMPLUS_HIGH) == TIMPLUS_HIGH)
	{
		TIMPLUSx->HIGH_PERIOD = Period;    //定时器周期配置值   实际周期值为配置值加1
	}	
}
