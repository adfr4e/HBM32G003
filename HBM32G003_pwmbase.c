...
* 功能说明:	PWMBASE公共参数初始化
* 输    入: PWMBASE_TypeDef * PWMBASEx                              指定要被设置的PWMBASE，有效值包括PWMBASE0
...
void PWMBASE_Common_Init(PWMBASE_TypeDef * PWMBASEx, PWMBASE_CommonInitTypeDef * PWMBASE_CommonInitStruct)
{
	...
	PWMBASEx->PERIOD = PWMBASE_CommonInitStruct->Period;                  //PWMBASE周期   实际周期是配置值加1
...
void PWMBASE_SetPeriod(PWMBASE_TypeDef * PWMBASEx, uint16_t Period)
{
	...
	PWMBASEx->PERIOD = Period;                    //实际周期是配置值加1
}
