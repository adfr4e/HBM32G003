...
void ADC_Init(ADC_InitStructure * ADC_initStruct)
{
	...
	assert_param(IS_ADC_VDDOPEN(ADC_initStruct->Adcvdd_Open));                 //检查输入的参数Adcvdd_Open是否合法  
	
	assert_param(IS_ADC_VREF(ADC_initStruct->Vref));                           //检查输入的参数Vref是否合法  
	...
	assert_param(IS_ADC_EOC_IEN(ADC_initStruct->Eoc_IEn));                     //检查输入的参数Eoc_IEn是否合法  
	...
	assert_param(IS_ADC_FIFO_OVF_IEN(ADC_initStruct->Ovf_IEn));                //检查输入的参数Ovf_IEn是否合法  
	...
	if(ADC_initStruct->Vref == VREF_EXT)
	{
		ADC_TRIM_Data = FLASH_Read_Word_2(TRIM_ADC_EXT_Addr);     //读取ADC外部参考TRIM值
	}
	else if(ADC_initStruct->Vref == VREF_IN)
	{
		ADC_TRIM_Data = FLASH_Read_Word_2(TRIM_ADC_IN_Addr);      //读取ADC内部参考TRIM值
	}
	
	ADC->CALIB_OFFSET = ADC_TRIM_Data & 0xFF;                     //写入OFFSET寄存器
	
	ADC->CALIB_KD = (ADC_TRIM_Data >> 16) & 0x3FF;                //写入KD寄存器
	...
	temp &= ~(0x1FF << ADC_CFG_CH0_POS);
	temp |= ADC_initStruct->Channels << ADC_CFG_CH0_POS;   //ADC转换通道选中，ADC_CH0、ADC_CH1、... ... 、ADC_CH8及其组合（即“按位或”运算）
	...
	temp &= ~(0x01 << ADC_CFG_EN_AVDDSNS_POS);
	temp |= ADC_initStruct->Adcvdd_Open << ADC_CFG_EN_AVDDSNS_POS;     //ADC VDD检测使能常开   0:只有通道8有效时,ADC VDD检测使能才打开  1:ADC VDD检测使能常开
	...
	temp &= ~(0x3F << ADC_EXTTRIG_SEL_TRIG0_POS);                  //选择触发源来触发ADC采样
	temp = (ADC_initStruct->ExtTrig_Sel << ADC_EXTTRIG_SEL_TRIG0_POS);
	
	ADC->EXTTRIG_SEL = temp;
	
	if(ADC_initStruct->Vref == VREF_EXT)                          //ADC参考源选择  0:外部参考  1:内部参考
	{
	...
	temp &= ~(ADC_IE_CH0EOC_MSK | ADC_IE_CH1EOC_MSK | ADC_IE_CH2EOC_MSK | ADC_IE_CH3EOC_MSK |
	...
	temp |=     (((ADC_initStruct->Eoc_IEn & ADC_CH0) ? 1 : 0) << ADC_IE_CH0EOC_POS) |
				...
				(((ADC_initStruct->Eoc_IEn & ADC_CH8) ? 1 : 0) << ADC_IE_CH8EOC_POS);       //通道数据转换完成中断使能  如果开启了多个通道转换，只需要开启最后一个通道的中断即可
	...
	temp &= ~(0x01 << ADC_IE_FIFO_OVF_POS);
	temp |= ADC_initStruct->Ovf_IEn << ADC_IE_FIFO_OVF_POS;          //FIFO 溢出中断使能
...
* 函数名称:	ADC_Read()
* 功能说明:	从指定通道读取转换结果
* 输    入: uint32_t chn			要读取转换结果的通道，有效值ADC_CH0、ADC_CH1、... ... 、ADC_CH8				
...
* 函数名称:	ADC_IsEOC()
* 功能说明:	指定通道是否转换完成
* 输    入: uint32_t chn			要查询状态的通道，有效值ADC_CH0、ADC_CH1、... ... 、ADC_CH8				
...
* 函数名称:	ADC_IntEOCEn()
* 功能说明:	转换完成中断使能
* 输    入: uint32_t chn			要设置的通道，有效值ADC_CH0、ADC_CH1、... ... 、ADC_CH8				
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
* 函数名称:	ADC_IntEOCStat()
* 功能说明:	转换完成中断状态
* 输    入: uint32_t chn			要查询的通道，有效值ADC_CH0、ADC_CH1、... ... 、ADC_CH8					
* 输    出: uint8_t				    1 该通道完成了转换    0 该通道未完成转换
* 注意事项: 无
******************************************************************************************************************************************/
...
* 功能说明:	FIFO溢出状态
* 输    入: 无		
* 输    出: uint8_t				1 FIFO溢出 0 FIFO未溢出
* 注意事项: 无
******************************************************************************************************************************************/
