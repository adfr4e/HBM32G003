...
typedef enum
{
	VREF_EXT =  0x00,        //ADC采用外部参考时钟
	VREF_IN  =  0x01         //ADC采用内部参考时钟
} ADC_Vref_TypeDef;
...
typedef struct {
	...
	uint16_t Channels;		               //ADC转换通道选中，ADC_CH0、ADC_CH1、... ... 、ADC_CH8及其组合（即“按位或”运算）
	...
	FunctionalState Adcvdd_Open;           //ADC VDD检测使能常开   0:只有通道8有效时,ADC VDD检测使能才打开  1:ADC VDD检测使能常开
	
	ADC_Vref_TypeDef Vref;                 //ADC参考源选择  0:外部参考  1:内部参考
	
	ADC_Trig_TypeDef Trig;                 //ADC触发源选择  0  CPU   1  外部信号
	
	uint8_t  ExtTrig_Sel;                  //外部信号触发源选择
	
	FunctionalState Offset_En;             //offset数据使能   0:不使能  1:使能
	
	FunctionalState Kd_En;                 //kd数据使能       0:不使能  1:使能
	
	uint8_t Eoc_IEn;		               //EOC中断使能，可针对每个通道设置，其有效值为ADC_CH0、ADC_CH1、... ... 、ADC_CH8及其组合（即“按位或”运算）
	
	FunctionalState Ovf_IEn;		       //FIFO 溢出中断使能
...
uint8_t ADC_OVFStat(void);                          //FIFO溢出状态
...
void ADC_IntOVFEn(void);                            //FIFO溢出中断使能
void ADC_IntOVFDis(void);                           //FIFO溢出中断禁止
uint8_t ADC_IntOVFStat(void);                       //FIFO溢出中断状态
void ADC_IntOVFClr(void);                           //FIFO溢出中断状态清除
