...
void PWMPLUS_CommonInit(PWMPLUS_TypeDef * PWMPLUSx, PWMPLUS_CommonInitTypeDef * PWMPLUS_CommonInitStruct)
{
	...
	assert_param(IS_PWMPLUS_BRAKE_LEVEL(PWMPLUS_CommonInitStruct->Brake_InputLevel));  //检查输入的参数Brake_InputLevel是否合法  
	...
	PWMPLUSx->PERIOD = PWMPLUS_CommonInitStruct->Period;           //PWMPLUS周期   实际周期为配置值加1
	...
	temp &= ~(0x03 << PWMPLUS_BRK_CTR_INLEV_POS);     //刹车输入有效电平配置
...
void PWMPLUS_ChannelInit(PWMPLUS_TypeDef * PWMPLUSx, PWMPLUS_ChannelInitTypeDef * PWMPLUS_ChannelInitStruct)
{
	...
	assert_param(IS_PWMPLUS_BRAKEEN(PWMPLUS_ChannelInitStruct->Brake_Enable));          //检查输入的参数Brake_Enable是否合法  
	...
	if((PWMPLUS_ChannelInitStruct->Channel & PWMPLUS_CH0) == PWMPLUS_CH0)
	{
		...
		temp &= ~(0x03 << PWMPLUS_BRK_CTR_CH0EN_POS);        //刹车使能
...
* 功能说明:	PWMPLUS时钟选择   
* 输    入: PWMPLUS_Clk_Src_TypeDef Clk_Src  
*			
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
* 功能说明:	设置刹车滤波
* 输    入: PWMPLUS_BrakeFilter_TypeDef Brake_Filter
*			
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
* 功能说明:	设置刹车输入有效电平选择 
* 输    入: PWMPLUS_BrakeLevel_TypeDef Brake_InputLevel
*			
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
void PWMPLUS_SetBrake_InputLevel(PWMPLUS_TypeDef * PWMPLUSx,PWMPLUS_BrakeLevel_TypeDef Brake_InputLevel)
{
	...
	temp &= ~(0x03 << PWMPLUS_BRK_CTR_INLEV_POS);           //刹车输入有效电平选择 
...
void PWMPLUS_SetBrake_Enable(PWMPLUS_TypeDef * PWMPLUSx,uint8_t CHx,PWMPLUS_BrakeEnable_TypeDef Brake_Enable)
{
	...
	assert_param(IS_PWMPLUS_BRAKE_ENABLE(Brake_Enable));                 //检查输入的参数Brake_Enable是否合法  
...
void PWMPLUS_SetPeriod(PWMPLUS_TypeDef * PWMPLUSx, uint16_t Period)
{
	...
	PWMPLUSx->PERIOD = Period;      //实际周期为配置值加1
}
...
void PWMPLUS_IntBrake1En(PWMPLUS_TypeDef * PWMPLUSx)
{
	...
	PWMPLUSx->IE |= (0x01 << PWMPLUS_IE_BRK1_POS);                       //刹车0中断使能
}
...
void PWMPLUS_IntBrake1Dis(PWMPLUS_TypeDef * PWMPLUSx)
{
	...
	PWMPLUSx->IE &= ~(0x01 << PWMPLUS_IE_BRK1_POS);                      //刹车0中断禁能
}
